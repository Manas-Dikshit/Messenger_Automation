...
class _FrozenDateTime(datetime):
    ...
    used to simulate the 23:50 IST workflow trigger time in tests."""
...
def freeze_time(monkeypatch):
    """Freeze birthday_sms.date_utils's notion of "now" to a given
    naive datetime (tzinfo is attached by the code under test)."""

    def _freeze(naive_dt: datetime) -> None:
        frozen_cls = type("_Frozen", (_FrozenDateTime,), {"_frozen_at": naive_dt})
        monkeypatch.setattr(date_utils_module, "datetime", frozen_cls)
