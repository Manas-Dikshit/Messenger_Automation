...
workflow" button. [cron-job.org](https://cron-job.org) (free) calls
that trigger on a precise schedule instead, so the actual send time
no longer depends on GitHub's scheduling queue.

Full step-by-step guide - GitHub token creation and permissions,
cron-job.org account and job setup, curl commands to self-test each
piece, and troubleshooting for common configuration mistakes:
...
happen to fire the same day. `cron_ping.yml` has no dedupe logic, so
it intentionally carries **no** `schedule:` trigger at all - see the
setup guide's note on this.
